...
namespace EBA
{
    internal partial class Facturacion : Form
    {
        ...
        //Cliente curClient;
        AutoCompleteStringCollection clientList = new AutoCompleteStringCollection();
        ...
        private void Facturacion_Load(object sender, EventArgs e)
        {
            ...
            foreach (Cliente c in factura.clientList)
                clientList.Add(c.apellido + ", " + c.nombre);
        ...
        private void setHeader()
        {
            ...
            dg[4, 4].Value = factura.numeroFactura;
            dg[4, 4].ReadOnly = true;
        ...
        internal void dg_cellEndEditEventHandler(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex > HEADER_LENGTH) //si intento saltearse alguna fila...
            {
                ...
                switch (e.ColumnIndex){
                    case 0:
                        if (!factura.tryAddProduct(codigo)) //todo lento
                    ...
                    case 2:
                        ...
                        if (!factura.tryAddProduct(nombre)) //todo rapido
                        {
                            removeLine(e.RowIndex);
                        }
                        break;
            ...
            if (e.ColumnIndex == 2)
            {
                if (e.RowIndex == 0) //cliente 
                {
                    if (dg[2, 0].Value != null)
                    {
                        //todo setearlo de la lista de clientes en lugar de hacer otra query
                        if (factura.trySetClient(dg[2, 0].Value.ToString()))
                        {
                        ...
                        else
                        {
                            ...
                            {
                                dg[2, 0].Value = factura.cliente.apellido + ", " + factura.cliente.nombre;
                                dg[2, 1].Value = factura.cliente.direccion;
                                dg[2, 2].Value = factura.cliente.iva;
                                dg[4, 1].Value = factura.cliente.telefono;
                                dg[4, 0].Value = DateTime.Today.ToShortDateString();
        ...
        private void updateProducts()
        {
            ...
            for (int i = HEADER_LENGTH + 1; i <= HEADER_LENGTH + factura.productList.Count + 1; i++)
            {
                ...
                {
                    dg.Rows[i].SetValues(new string[dg.ColumnCount]);
                    continue;
                }
                ...
                if (n < 1)
                    dg.Rows[i].SetValues(new string[dg.ColumnCount]);
             //todo: de esta forma si borro algo, no me lo saca de la lista.. deberia chequear si sigue en la lista
            }
            ...
            if (isCellEditable())
            {
                ...
                switch (dg.CurrentCell.ColumnIndex)
                {
                    case 2:
                        ...
                        {
                            ((TextBox)e.Control).AutoCompleteCustomSource = clientList;
                        }
                        else
                        {
        ...
        private bool isCellEditable(int c, int r)
        {
            ...
            if (r > HEADER_LENGTH && c < 3 && r < 32) return true; //todo
